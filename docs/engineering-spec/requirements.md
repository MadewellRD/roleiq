...
- **NF5 — Deterministic, offline-testable core logic.** The pure logic (candidate ID hashing, provider selection, JSON repair, file extraction, competency ordering, wizard step derivation) must be unit-testable without network access or an API key. 95 such tests exist and pass as of this writing (`pytest -q`).
